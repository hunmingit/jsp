...
public class BoardServiceImpl implements BoardService{
    ...
    @Override
    public PageMaker PageMaker(int page) {
    ...
    public int todayLikeCount(String pId) {
    	 return boardDAO.todayLikeCount(pId);
    }
    @Override
    public boolean likeBoard(String pId, int bIdx) {
    	//오늘 좋아요 수 조회
    	int todayCount = boardDAO.todayLikeCount(pId);
    	//3 이상이면 false
    	if(todayCount >=3) {
    		return false;
    	}
    	//좋아요 로그 저장
    	boardDAO.insertBoardLike(pId, bIdx);
    	//게시글 좋아요수 증가
    	boardDAO.increaseBoardLike(bIdx);
